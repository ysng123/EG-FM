...
def save_model(args, model_without_ddp, optimizer, epoch, epoch_name=None):
    ...
    if hasattr(model_without_ddp, "build_training_checkpoint"):
        to_save = model_without_ddp.build_training_checkpoint(args=args, optimizer=optimizer, epoch=epoch)
    else:
        args_state = dict(vars(args)) if hasattr(args, "__dict__") else args
        to_save = {
