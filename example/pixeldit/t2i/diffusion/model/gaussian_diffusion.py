...
from egfm import make_training_batch
...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, timestep, model_kwargs=None, noise=None, skip_noise=False):
        ...
        if skip_noise:
            ...
            if noise is None:
                noise = th.randn_like(x_start)
            if self.flow and self.egfm_path is not None:
                clean_time = th.as_tensor(
                    self.alphas, device=t.device, dtype=x_start.dtype
                )[t]
                flow_batch = make_training_batch(
                    x_start, clean_time, self.egfm_path, noise=noise
                )
                x_t = flow_batch.state
                # PixelDiT T2I parameterizes time from clean to noise, while
                # EG-FM parameterizes it from noise to clean.
                flow_target = -flow_batch.velocity
            else:
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            target = {
                ...
                ModelMeanType.FLOW_VELOCITY: flow_target if flow_target is not None else noise - x_start,
