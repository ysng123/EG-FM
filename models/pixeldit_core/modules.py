...
class RotaryAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, pos, mask=None) -> torch.Tensor:
        ...
        qkv = self.qkv(x).reshape(bsz, seq_len, 3, self.num_heads, channels // self.num_heads).permute(2, 0, 1, 3, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]
        q = self.q_norm(q)
        k = self.k_norm(k)
        q, k = apply_rotary_emb(q, k, freqs_cis=pos)
        q = q.view(bsz, -1, self.num_heads, channels // self.num_heads).transpose(1, 2)
        k = k.view(bsz, -1, self.num_heads, channels // self.num_heads).transpose(1, 2).contiguous()
        v = v.view(bsz, -1, self.num_heads, channels // self.num_heads).transpose(1, 2).contiguous()
